...
def cra_3dmax():
    ...
    sleep_time = [30, 40, 30, 10]  # 各图片的等待时间
    grayscale = [True, True, True, True]  # 各图片是否使用灰度搜索
    skewing = [[0, 0], [0, 0], [0, 0], [0, 0]]  # x、y坐标偏移
    png_file_name = "_shot1"
    
    if install_from_png(app_name="3DMAX2020", confidence=0.8, sleep_time_list=sleep_time, grayscale_list=grayscale,
            ...
            if install_from_png(app_name="3DMAX2020", confidence=0.8, sleep_time_list=sleep_time, grayscale_list=grayscale,
                ...
                coordinate_list = install_from_png(app_name="3DMAX2020", confidence=0.8, sleep_time_list=sleep_time,
                ...
                if len(coordinate_list) == 2:
                    ...
                    key_soft = Application().start(join(getcwd(), 'app_pkg', '3DMAX2020', 'crack', 'crack.exe'))  # 打开注册机
                    
                    step = {0: ["Edit0", paste(), 'edit', 10],  # 将申请号粘贴
